...
import { AnimatePresence, m, Reorder, useReducedMotion } from "framer-motion";
...
// Stable module-scope default so an unset `history` prop doesn't hand a fresh
// `[]` to hooks every render (which would bust their dependency arrays).
...
// Stable empty snapshot for the queue ref's initial value. The ref is
// (re)populated from the live queue inside an effect, so this constant is only
// ever read in the brief pre-first-effect window (before any handler or the
// auto-dispatch edge can run) — keeping the ref's provenance free of
...
// ─── File preview strip ───────────────────────────────────────────────────
// The attached-files preview row that sits above the textarea. The outer
// `m.div` uses `layout` (not an explicit height keyframe) to animate the row
// collapsing the component height as files appear / disappear — animating a
// layout property like `height` would thrash layout every frame. The inner
// `mode="popLayout"` AnimatePresence pulls a removing tile out of layout flow
// so siblings can slide into the gap without fighting its exit anim. Keys are
// purely file-identity (no index) so removing the first file doesn't re-key —
// and remount — every surviving sibling.
...
function FilePreviewStrip({ files, size, onRemove }: FilePreviewStripProps) {
  return (
...
// ─── Queue region ─────────────────────────────────────────────────────────
// Reorderable queue rows above the textarea. The AnimatePresence stays mounted
// unconditionally (with the render condition *inside* it) so it can observe the
// last row leaving and run its exit animation. The outer `m.div` uses `layout`
// to collapse the region height when the queue empties; the Reorder.Group
// handles drag-reorder (top = next to dispatch) and the inner AnimatePresence
// handles per-row enter/exit.
interface QueueRegionProps {
  queue: QueuedMessage[];
  show: boolean;
  reduceMotion: boolean;
  isTouch: boolean;
  onReorder: (next: QueuedMessage[]) => void;
  onEdit: (item: QueuedMessage) => void;
  onRemove: (item: QueuedMessage) => void;
  onMove: (item: QueuedMessage, dir: -1 | 1) => void;
}
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          {mode === "stop" ? (
            ...
            // Override icon-sm's small 14px svg — the send glyph reads better a
            // touch larger. `size` matches the attribute to the CSS so the svg
            // box stays centered.
...
// ─── Composer row ─────────────────────────────────────────────────────────
// The auto-sizing textarea plus the bottom action bar (left slot, right slot,
// and the send/queue/stop button). The textarea ref is forwarded so the parent
// can focus it and measure/resize it.
interface ComposerRowProps {
  value: string;
  onValueChange: (value: string) => void;
  /** Exit history mode on real typing (recall sets value programmatically). */
  onHistoryReset: () => void;
...
const ComposerRow = forwardRef<HTMLTextAreaElement, ComposerRowProps>(
  (
    {
      value,
      onValueChange,
    ...
    return (
      <>
        <textarea
          ...
          // overwrite these).
          onFocus={(e) => {
            if (e.target.matches(":focus-visible")) onFocusVisibleChange(true);
            propsOnFocus?.(e);
          }}
          onBlur={(e) => {
...
ComposerRow.displayName = "ComposerRow";

// ─── Hooks ────────────────────────────────────────────────────────────────

// Auto-grow the textarea between `minRows` and `maxRows`. Parsed line-height is
// cached per textarea element — getComputedStyle on every keystroke is needless
// work when the value only changes with font or zoom changes.
...
interface UseSentHistoryParams {
  textareaRef: React.RefObject<HTMLTextAreaElement | null>;
  value: string;
  onValueChange: (value: string) => void;
  history: string[];
...
// Sent-message history navigation (readline-style). `historyIndex` is null when
// not browsing; `draftBeforeHistory` stashes the in-progress text so ArrowDown
...
function useSentHistory({
  ...
  // Handles plain ArrowUp/ArrowDown history recall and returns true when it
  // consumed the event (so the caller can skip its own key handling). Only
  // plain arrows (no modifiers) navigate, and only when the caret is on the
  // first/last line so multi-line editing still works normally.
  const handleHistoryKeyDown = useCallback(
    ...
    [history, value, historyIndex, onValueChange, setCaretEnd]
  );

  return { setHistoryIndex, handleHistoryKeyDown };
}
...
function useMessageQueue({
  ...
  // before the controlled `queue` prop round-trips back). Written from an
  // effect (never during render) so discarded renders can't leak a value.
  const queueRef = useRef<QueuedMessage[]>(EMPTY_QUEUE);
  useEffect(() => {
  ...
  // Auto-dispatch: on the streaming → idle edge (whether the response finished
  // on its own or the user pressed Stop), fire the head of the queue and drop
  // it. The consumer is expected to set status back to "streaming" inside
  // onSend, which re-arms this for the next item.
  const prevStatusRef = useRef(status);
  useEffect(() => {
    const prev = prevStatusRef.current;
    prevStatusRef.current = status;
    if (!supportsQueue) return;
    if (prev === "streaming" && status === "idle" && queueRef.current.length > 0) {
  ...
  }, [status, supportsQueue, onQueueChange, onSend]);

  const enqueue = useCallback(
    (item: QueuedMessage) => onQueueChange?.([...queueRef.current, item]),
    [onQueueChange]
  );
...
const InputMessage = forwardRef<HTMLDivElement, InputMessageProps>(
  (
    ...
    const {
      ...
      handleDrop,
      handleFileInputChange,
    } = useFileAttachments({ files, onFilesChange, accept, maxFiles, disabled });

    const { setHistoryIndex, handleHistoryKeyDown } = useSentHistory({
      textareaRef,
      value,
      onValueChange,
      history,
    ...
    const handleKeyDown = useCallback(
      ...
      [handleHistoryKeyDown, handleSend]
    );
    ...
    return (
      <div
        ref={ref}
        // Presentational wrapper: this mousedown only refocuses the real control
        // (the textarea within), so mark it `presentation` — it isn't itself a
        // control and shouldn't add a tab stop or announce as interactive.
        role="presentation"
      ...
      >
        <SurfaceProvider value={2}>
          ...
          <ComposerRow
            ...
            buttonDisabled={buttonMode === "stop" ? !!disabled : !canSend}
            reduceMotion={reduceMotion}
            onActivate={buttonMode === "stop" ? handleStop : handleSend}
          />
