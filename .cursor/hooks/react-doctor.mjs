...
const main = () => {
  ...
  const message = `React Doctor found issues in the changed files. Project gate: score must be 100/100. Fix all confirmed issues and re-run \`npm run doctor\` until the score is 100 before finishing. Do not disable rules to inflate the score unless the user explicitly asks.\n\n${scanOutput}`;
